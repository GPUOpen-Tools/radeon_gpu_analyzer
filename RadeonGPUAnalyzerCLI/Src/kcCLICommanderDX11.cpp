...
void kcCLICommanderDX::RunCompileCommands(const Config& config, LoggingCallBackFuncP callback)
{
    ...
    if (isInitSuccessful)
    {
        ...
        // Check flags first.
        if (config.m_mode == Mode_DX11 && config.m_Profile.length() == 0 && !config.m_DxbcInputDx11)
        {
            std::stringstream s_Log;
...
bool kcCLICommanderDX::Compile(const Config& config, const GDT_GfxCardInfo& gfxCardInfo, string sDevicenametoLog)
{
    ...
    if (config.m_mode != Mode_AMDIL && !config.m_DxbcInputDx11 &&
    ...
    if (ret)
    {
        ...
        else
        {
            ...
            beRet = m_pBackEndHandler->theOpenDXBuilder()->Compile(config.m_mode,
                sSource, dxOptions, config.m_DxbcInputDx11);
