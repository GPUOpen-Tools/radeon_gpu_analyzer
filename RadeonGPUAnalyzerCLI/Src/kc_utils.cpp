...
{
    // Call the backend.
    beStatus rc = BeStaticIsaAnalyzer::GenerateControlFlowGraph(isa_file_name, output_filename, per_inst_cfg, printCmd);
...
bool KcUtils::GenerateControlFlowGraph(const std::string& isa_filename, const std::string& output_filename,
    LoggingCallbackFunction pCallback, bool per_inst_cfg, bool print_cmd)
{
    ...
    // Invoke the routine.
    return GenerateControlFlowGraph(isa_name_gtstr, output_filename_gtstr, pCallback, per_inst_cfg, print_cmd);
}
...
{
    std::cout << kStrRgaProductName << " " << kStrRgaVersionPrefix << STR_RGA_VERSION << "." << STR_RGA_BUILD_NUM << std::endl;
}
...
void KcUtils::CheckForUpdates()
{
    ...
    std::string build_date_string(STR_RGA_BUILD_DATE);

    if (build_date_string == STR_RGA_BUILD_DATE_DEV)
    {
        // Pretend a dev build has no version so that
        // all public versions are reported as being newer.
        rga_cli_version.m_major = 0;
        rga_cli_version.m_minor = 0;
        rga_cli_version.m_patch = 0;
        rga_cli_version.m_build = 0;
    }
    else
    {
        rga_cli_version.m_major = RGA_VERSION_MAJOR;
        rga_cli_version.m_minor = RGA_VERSION_MINOR;
        rga_cli_version.m_patch = RGA_VERSION_UPDATE;
        rga_cli_version.m_build = 0;
    }
    ...
    bool checked_for_update = UpdateCheck::CheckForUpdates(rga_cli_version, STR_RGA_UPDATECHECK_URL, STR_RGA_UPDATECHECK_ASSET_NAME, update_info, error_message);
    ...
    {
        if (!update_info.m_isUpdateAvailable)
        {
        ...
        else
        {
            ...
            for (std::vector<UpdateCheck::ReleaseInfo>::const_iterator release_iter = update_info.m_releases.cbegin(); release_iter != update_info.m_releases.cend(); ++release_iter)
            {
                std::cout << "Description: " << release_iter->m_title << endl;
                std::cout << "Version: " << release_iter->m_version.ToString() << " (" << UpdateCheck::ReleaseTypeToString(release_iter->m_type) << ")" << endl;
                std::cout << "Released: " << release_iter->m_date << endl;

                if (!release_iter->m_tags.empty())
                {
                    std::cout << "Tags: " << release_iter->m_tags[0];
                    for (uint32_t i = 1; i < release_iter->m_tags.size(); ++i)
                    {
                        std::cout << ", " << release_iter->m_tags[i];
                    }
                    std::cout << endl;
                }

                if (release_iter->m_downloadLinks.size() > 0)
                {
                    std::cout << "Download a package from:" << endl;

                    for (size_t package_index = 0; package_index < release_iter->m_downloadLinks.size(); ++package_index)
                    {
                        std::cout << "   " << release_iter->m_downloadLinks[package_index].m_url << endl;
                    }
                }

                if (release_iter->m_infoLinks.size() > 0)
                {
                    std::cout << "For more information, visit:" << endl;

                    for (size_t infoLinkIndex = 0; infoLinkIndex < release_iter->m_infoLinks.size(); ++infoLinkIndex)
                    {
                        std::cout << "   " << release_iter->m_infoLinks[infoLinkIndex].m_url << endl;
                    }
                }
