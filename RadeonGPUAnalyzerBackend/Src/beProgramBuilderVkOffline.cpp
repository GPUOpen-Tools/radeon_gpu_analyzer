...
static const std::string  STR_AMDSPV_DEVICE_GFX1012 = "1012";
...
static bool GetGfxIpForVulkan(AMDTDeviceInfoUtils* pDeviceInfo, const VkOfflineOptions& vulkanOptions, std::string& gfxIpStr)
{
             ...
             vulkanOptions.m_targetDeviceName.compare(DEVICE_NAME_GFX906) == 0)
    {
        ...
        ret = !gfxIpStr.empty();
             ...
             vulkanOptions.m_targetDeviceName.compare(DEVICE_NAME_GFX1012) == 0)
    {
        // Special case #4: gfx10 devices.
        gfxIpStr =
            vulkanOptions.m_targetDeviceName == DEVICE_NAME_GFX1010 ? STR_AMDSPV_DEVICE_GFX1010 :
            vulkanOptions.m_targetDeviceName == DEVICE_NAME_GFX1012 ? STR_AMDSPV_DEVICE_GFX1012 :
            "";
        ret = !gfxIpStr.empty();
    }
    else
...
static void MergeShaderOutput(const VkOfflineOptions& options, const gtString& fromDisassemblyFile, const gtString& toDisassemblyFile,
    const gtString& fromStatsFile, const gtString& toStatsFile, const gtString& fromBinaryFile, const gtString& toBinaryFile)
{
    // Disassembly.
    if (options.m_isAmdIsaDisassemblyRequired)
    {
        std::string txt;
        bool isDisassemblyFileRead = kcUtils::ReadTextFile(fromDisassemblyFile.asASCIICharArray(), txt, nullptr);
        assert(isDisassemblyFileRead);
        assert(!toDisassemblyFile.isEmpty());
        if (isDisassemblyFileRead && !toDisassemblyFile.isEmpty())
        {
            kcUtils::WriteTextFile(toDisassemblyFile.asASCIICharArray(), txt, nullptr);
        }
    }
    ...
    {
        std::string txtStats;
        bool isStatsFileRead = kcUtils::ReadTextFile(fromStatsFile.asASCIICharArray(), txtStats, nullptr);
        assert(isStatsFileRead);
        assert(!toStatsFile.isEmpty());
        if (isStatsFileRead && !toStatsFile.isEmpty())
        {
            kcUtils::WriteTextFile(toStatsFile.asASCIICharArray(), txtStats, nullptr);
        }
    }
    ...
    if (options.m_isAmdIsaBinariesRequired)
    {
        ...
        bool isBinFileRead = beUtils::ReadBinaryFile(fromBinaryFile.asASCIICharArray(), binaryContent);
        assert(isBinFileRead);
        assert(!toBinaryFile.isEmpty());
        if (isBinFileRead && !toBinaryFile.isEmpty())
        {
            kcUtils::WriteBinaryFile(toBinaryFile.asASCIICharArray(), binaryContent, nullptr);
        }
    }
}
...
static bool VerifyAmdspvOutput(const VkOfflineOptions& options, const std::string& amdspvGfxip)
{
    bool  ret = true;
    ...
    if (!amdspvGfxip.empty() && amdspvGfxip[0] != '1' && amdspvGfxip[0] != '9')
    {
        if (options.m_isAmdIsaDisassemblyRequired)
        {
    ...
    {
        if (!options.m_pipelineShaders.m_geometryShader.isEmpty() && options.m_pipelineShaders.m_tessEvaluationShader.isEmpty() &&
            !beUtils::IsFilePresent(options.m_isaDisassemblyOutputFiles.m_geometryShader.asASCIICharArray()))
        {
            // Geometry is present, but tessellation control is not present - geometry merged with vertex.
            MergeShaderOutput(options,
                options.m_isaDisassemblyOutputFiles.m_vertexShader,
                options.m_isaDisassemblyOutputFiles.m_geometryShader,
                options.m_scStatisticsOutputFiles.m_vertexShader,
                options.m_scStatisticsOutputFiles.m_geometryShader,
                options.m_isaBinaryFiles.m_vertexShader,
                options.m_isaBinaryFiles.m_geometryShader);
            std::cout << STR_INFO_VULKAN_GEOM_VERT_MERGED << std::endl;
        }
        else if (!options.m_pipelineShaders.m_geometryShader.isEmpty() && !options.m_pipelineShaders.m_tessEvaluationShader.isEmpty())
        {
            // Geometry and tessellation evaluation both present - geometry merged with tessellation evaluation.
            MergeShaderOutput(options,
                options.m_isaDisassemblyOutputFiles.m_tessEvaluationShader,
                options.m_isaDisassemblyOutputFiles.m_geometryShader,
                options.m_scStatisticsOutputFiles.m_tessEvaluationShader,
                options.m_scStatisticsOutputFiles.m_geometryShader,
                options.m_isaBinaryFiles.m_tessEvaluationShader,
                options.m_isaBinaryFiles.m_geometryShader);
            std::cout << STR_INFO_VULKAN_GEOM_TESE_MERGED << std::endl;
        }

        if (!options.m_pipelineShaders.m_tessControlShader.isEmpty() && !options.m_pipelineShaders.m_vertexShader.isEmpty())
        {
            // Tessellation control is present - tessellation control merged with vertex.
            MergeShaderOutput(options,
                options.m_isaDisassemblyOutputFiles.m_vertexShader,
                options.m_isaDisassemblyOutputFiles.m_tessControlShader,
                options.m_scStatisticsOutputFiles.m_vertexShader,
                options.m_scStatisticsOutputFiles.m_tessControlShader,
                options.m_isaBinaryFiles.m_vertexShader,
                options.m_isaBinaryFiles.m_tessControlShader);
        ...
        if (isDeviceHwGenExtracted && !deviceGfxIp.empty())
        {
            ...
            if ((ret = AddInputFileNames(vulkanOptions, cmd)) == beKA::beStatus_SUCCESS)
            {
                ...
                if (isLaunchSuccess)
                {
                    ...
                    else if (!VerifyAmdspvOutput(vulkanOptions, deviceGfxIp))
                    {
