...
private:
    ...
    // The device names.
    std::set<string>                 m_DeviceNames;

    // Map from OpenCL device ID to OpenCL device name.
    std::map<cl_device_id, string>   m_DeviceIdNameMap;

    // Map from OpenCL device name to OpenCL device ID.
    std::map<string, cl_device_id>   m_NameDeviceIdMap;

    // Map from OpenCL device name to device type.
    std::map<string, cl_device_type> m_NameDeviceTypeMap;

    // Map from device name to ISA Code Object disassembly.
    std::map<string, std::string> m_DeviceToCoDisassembly;

    // Map from device name to whole Code Object disassembly.
    std::map<string, std::string> m_DeviceToCoDisassemblyWhole;
