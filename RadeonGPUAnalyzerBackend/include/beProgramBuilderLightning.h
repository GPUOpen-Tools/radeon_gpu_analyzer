...
struct KernelCodeProps
{
    ...
    size_t  workgroupGroupSegmentSize;
    size_t  isaSize;
};
...
public:
    ...
    static beStatus GetCompilerVersion(SourceLanguage lang, std::string& outText);
    ...
    static beStatus CompileOpenCLToBinary(const OpenCLOptions&              userOptions,
    ...
    static beStatus PreprocessOpenCL(const std::string& inputFile, const std::string& args, std::string& output);

    // Disassemble binary to ISA text.
    static beStatus  DisassembleBinary(const std::string& binFileName, const std::string& device,
                                       bool lineNumbers, std::string& outISAText, std::string& errMsg);

    // Extract CodeObject metadata.
    static beStatus  ExtractMetadata(const std::string& binFileName, std::string& metadataText);

    // Extract CodeObject Metadata kernels properties.
    static beStatus  ExtractKernelCodeProps(const std::string& binFileName, CodePropsMap& codeProps);

    // Extract names of kernels from the binary.
    static beStatus  ExtractKernelNames(const std::string& binFileName, std::vector<std::string>& kernelNames);
...
protected:
    ...
    static beStatus AddCompilerStandardOptions(beKA::SourceLanguage lang, std::string& options);

    // Builds a string containing all necessary options for OpenCL compiler.
    // Writes constructed options string to "options".
    static beStatus ConstructOpenCLCompilerOptions(const OpenCLOptions&              userOptions,
                                                   const std::vector<std::string>&   srcFileNames,
                                                   const std::string&                binFileName,
                                                   const std::string&                device,
                                                   bool                              dumpIL,
    ...
    static beStatus InvokeCompiler(beKA::SourceLanguage lang, const std::string& cmdLineOptions,
                                   std::string& stdOut, std::string& stdErr, unsigned long timeOut = 0);
    ...
    static beStatus InvokeObjDump(ObjDumpOp op, const std::string& cmdLineOptions, std::string& outText, std::string& errMsg);
