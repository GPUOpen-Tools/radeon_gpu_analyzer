...
rgFileMenuTitlebar::rgFileMenuTitlebar(QWidget* pParent) :
    QWidget(pParent)
{
    ...
    connect(m_pTitleTextEdit, &QLineEdit::editingFinished, this, &rgFileMenuTitlebar::StopEditing);
    connect(m_pTitleTextEdit, &QLineEdit::returnPressed, this, &rgFileMenuTitlebar::HandleReturnPressed);
...
rgFileMenuTitlebar::~rgFileMenuTitlebar()
{
...
void rgFileMenuTitlebar::SetTitle(const std::string& title)
{
    // Set widget text.
...
void rgFileMenuTitlebar::StartEditing()
{
...
void rgFileMenuTitlebar::StopEditing()
{
    ...
    if (rgUtils::IsValidFileName(updatedProjectName))
    {
        ...
        if (labelText.compare(updatedProjectName) != 0)
        {
            ...
            updatedTitle << rgUtils::GetProjectTitlePrefix(OpenCL) << updatedProjectName;
...
void rgFileMenuTitlebar::mouseDoubleClickEvent(QMouseEvent* pEvent)
{
...
void rgFileMenuTitlebar::keyPressEvent(QKeyEvent* pEvent)
{
...
void rgFileMenuTitlebar::RefreshTooltip(const std::string& updatedTooltip)
{
...
void rgFileMenuTitlebar::HandleReturnPressed()
{
    m_pTitleTextEdit->blockSignals(true);
...
void rgFileMenuTitlebar::SetCursor()
{
    // Set mouse cursor to pointing hand cursor.
    setCursor(Qt::PointingHandCursor);
}

void rgFileMenuTitlebar::ResizeTooltipBox(const std::string& tooltip)
{
