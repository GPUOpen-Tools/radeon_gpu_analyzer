...
rgEditorElementArrayElement::rgEditorElementArrayElement(QWidget* pParent, const std::string& memberName)
    : rgEditorElement(pParent, memberName, rgEditorDataType::ArrayElement)
{
    m_pEditorWidget = new rgPipelineStateEditorWidgetArrayElement(this);
...
rgPipelineStateEditorWidget* rgEditorElementArrayElement::GetEditorWidget()
{
    return m_pEditorWidget;
}

void rgEditorElementArrayElement::ConnectSignals()
{
    // Connect the delete button handler.
    bool isConnected = connect(m_pEditorWidget, &rgPipelineStateEditorWidgetArrayElement::DeleteButtonClicked,
        this, &rgEditorElementArrayElement::HandleDeleteButtonClicked);
    assert(isConnected);

    // Connect the loss of focus handler.
    isConnected = connect(m_pEditorWidget, &rgPipelineStateEditorWidgetArrayElement::FocusOutSignal,
        this, &rgEditorElement::HandleEditorFocusOut);
    assert(isConnected);
}

void rgEditorElementArrayElement::SetElementIndex(rgEditorElementArray* pParentArray, int childIndex)
{
...
void rgEditorElementArrayElement::HandleDeleteButtonClicked()
{
