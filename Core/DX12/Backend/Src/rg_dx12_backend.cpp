...
namespace rga
{
    ...
    public:
        ...
        bool CreateStateObject(const D3D12_STATE_OBJECT_DESC* ray_tracing_state_object, const std::wstring& raygen_shader_name,
            std::vector<std::shared_ptr<RgDx12ShaderResultsRayTracing>>& raytracing_shader_stats, std::vector<unsigned char>& pipeline_binary,
            bool& is_unified_mode, std::vector<std::string>& indirect_shader_names, std::string& error_msg) const;
        ...
        // Creates the state object when no export is given (extracts results for the N generated pipelines).
        bool CreateStateObjectAll(const RgDx12Config& config, const D3D12_STATE_OBJECT_DESC* ray_tracing_state_object,
            std::vector<std::shared_ptr<RgDx12ShaderResultsRayTracing>>& raytracing_shader_stats,
            std::vector<std::shared_ptr<std::vector<unsigned char>>>& pipeline_binary, std::string& error_msg) const;

    private:
        HMODULE LoadUMDLibrary();
        HMODULE amd_d3d_dll_handle_ = NULL;
        IAmdExtD3DFactory* amd_ext_object_ = NULL;
        IAmdExtD3DShaderAnalyzer1* amd_shader_analyzer_ext_ = NULL;
    };
    ...
    HMODULE RgDx12Backend::Impl::LoadUMDLibrary()
{
    ...
    bool RgDx12Backend::Impl::CreateStateObject(const D3D12_STATE_OBJECT_DESC* ray_tracing_state_object,
        const std::wstring& raygen_shader_name, std::vector<std::shared_ptr<RgDx12ShaderResultsRayTracing>>& raytracing_shader_stats,
        std::vector<unsigned char>& pipeline_binary, bool& is_unified_mode, std::vector<std::string>& indirect_shader_names,
        std::string& error_msg) const
    {
        assert(amd_shader_analyzer_ext_ != nullptr);
        bool ret = amd_shader_analyzer_ext_ != nullptr;
        if (ret)
        {
            std::string raygen_shader_nameAsStr = RgDx12Utils::wstrToStr(raygen_shader_name);
            AmdExtD3DPipelineHandle* pipeline_handle = new AmdExtD3DPipelineHandle{};
            ...
            if (ret)
            {
                ...
                is_unified_mode = !amd_shader_analyzer_ext_->IsRayTracingPipelineIndirect(*pipeline_handle);
                if (is_unified_mode)
                {
                    std::shared_ptr< RgDx12ShaderResultsRayTracing> pCurrItem = std::make_shared<RgDx12ShaderResultsRayTracing>();
                    raytracing_shader_stats.push_back(pCurrItem);
                    ...
                    hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIsaDisassembly(*pipeline_handle,
                        raygen_shader_name.c_str(), nullptr, &diassembly_bytes);
                    assert(SUCCEEDED(hr));
                    ret = (SUCCEEDED(hr));
                    if (ret && diassembly_bytes > 0)
                    {
                        pCurrItem->disassembly = new char[diassembly_bytes] {};
                        hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIsaDisassembly(*pipeline_handle,
                            raygen_shader_name.c_str(), pCurrItem->disassembly, &diassembly_bytes);
                        assert(SUCCEEDED(hr));
                        assert(pCurrItem->disassembly != nullptr);
                        if (!SUCCEEDED(hr) || pCurrItem->disassembly == nullptr)
                        {
                            std::stringstream msg;
                            msg << kStrErrorDisassemblyContentsExtractionFailure <<
                                kStrErrorPipelineIdentifiedByRaygenShaderName << raygen_shader_nameAsStr;
                    ...
                    {
                        std::stringstream msg;
                        std::string shaderNameStr = RgDx12Utils::wstrToStr(raygen_shader_name);
                        msg << kStrErrorDisassemblySizeExtractionFailure <<
                            kStrErrorPipelineIdentifiedByRaygenShaderName << shaderNameStr;
                    ...
                    if (ret)
                    {
                        ...
                        hr = amd_shader_analyzer_ext_->GetRayTracingPipelineStats(*pipeline_handle,
                            raygen_shader_name.c_str(), &rt_stats_driver);
                        assert(SUCCEEDED(hr));
                        if (SUCCEEDED(hr))
                        {
                            ExtractRayTracingStats(*pCurrItem, rt_stats_driver);
                        }
                        else
                        {
                            std::stringstream msg;
                            ...
                            if (buffer_size > 0 && SUCCEEDED(hr))
                            {
                                ...
                                if (pipeline_binary.empty())
                                {
                                    ...
                                    msg << kStrErrorPipelineBinaryExtractionFailure <<
                                        kStrErrorPipelineIdentifiedByIndexPipelineBinary << raygen_shader_nameAsStr;
                                }
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    indirect_shader_names.clear();
                    uint32_t shader_count = 0;
                    hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIndirectShaderCount(*pipeline_handle, &shader_count);
                    assert(shader_count > 0);
                    for (uint32_t shaderIndex = 0; shaderIndex < shader_count; shaderIndex++)
                    {
                        // Retrieve shader name.
                        size_t name_bytes = 0;
                        hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIndirectShaderName(*pipeline_handle, shaderIndex, &name_bytes, nullptr);
                        assert(SUCCEEDED(hr));
                        if (SUCCEEDED(hr))
                        {
                            std::string curr_shader_name;
                            curr_shader_name.resize(name_bytes);
                            hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIndirectShaderName(*pipeline_handle, shaderIndex, &name_bytes, (char*)curr_shader_name.data());
                            if (!curr_shader_name.empty())
                            {
                                indirect_shader_names.push_back(curr_shader_name);
                                std::shared_ptr<RgDx12ShaderResultsRayTracing> curr_results = std::make_shared<RgDx12ShaderResultsRayTracing>();
                                std::wstring curr_shader_name_wide = RgDx12Utils::strToWstr(curr_shader_name);
                                assert(!curr_shader_name_wide.empty());
                                if (!curr_shader_name_wide.empty())
                                {
                                    ...
                                    if (ret && diassembly_bytes > 0)
                                    {
                                        ...
                                        if (!SUCCEEDED(hr) || curr_results->disassembly == nullptr)
                                        {
                                            std::stringstream msg;
                                            msg << kStrErrorDisassemblyContentsExtractionFailure <<
                                                kStrErrorPerShaderName << curr_shader_name.c_str();
                                            error_msg.append(msg.str());
                                            ret = false;
                                        }
                                    }
                                    ...
                                    raytracing_shader_stats.push_back(curr_results);
                                }
                            }
                            else
                            {
                                std::stringstream msg;
                                msg << kStrErrorDxrEmptyShaderName << shaderIndex;
                                error_msg.append(msg.str());
                                ret = false;
                            }
                        }
                        ...
                        {
                            std::stringstream msg;
                            msg << kStrErrorDxrFailedToRetrieveShaderName << shaderIndex;
                            error_msg.append(msg.str());
                            ret = false;
                        }
                    }
    ...
    bool RgDx12Backend::Impl::CreateStateObjectAll(const RgDx12Config& config, const D3D12_STATE_OBJECT_DESC* ray_tracing_state_object,
        std::vector<std::shared_ptr<RgDx12ShaderResultsRayTracing>>& raytracing_shader_stats,
        std::vector<std::shared_ptr<std::vector<unsigned char>>>& pipeline_binaries, std::string& error_msg) const
    {
        assert(amd_shader_analyzer_ext_ != nullptr);
        bool ret = amd_shader_analyzer_ext_ != nullptr;
        if (ret)
        {
            ...
            if (ret)
            {
                // Retrieve the number of pipelines that were created.
                uint32_t pipeline_count = 0;
                hr = amd_shader_analyzer_ext_->GetRayTracingPipelineElfCount(*pipeline_handle, &pipeline_count);
                assert(SUCCEEDED(hr));
                assert(pipeline_count > 0);
                ret = (SUCCEEDED(hr) && pipeline_count > 0);
                if (ret && pipeline_count > 0)
                {
                    bool is_disassembly_required = !config.dxr_isa_output.empty();
                    bool is_statistics_required = !config.dxr_statistics_output.empty();
                    for (uint32_t i = 0; i < pipeline_count; i++)
                    {
                        std::shared_ptr<RgDx12ShaderResultsRayTracing> curr_results =
                            std::make_shared<RgDx12ShaderResultsRayTracing>();

                        if (is_disassembly_required)
                        {
                            // Retrieve the disassembly for the current pipeline.
                            size_t diassembly_bytes = 0;
                            hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIsaDisassemblyByIndex(*pipeline_handle,
                                i, nullptr, &diassembly_bytes);
                            assert(SUCCEEDED(hr));
                            ret = (SUCCEEDED(hr));
                            if (ret && diassembly_bytes > 0)
                            {
                                curr_results->disassembly = new char[diassembly_bytes] {};
                                hr = amd_shader_analyzer_ext_->GetRayTracingPipelineIsaDisassemblyByIndex(*pipeline_handle,
                                    i, curr_results->disassembly, &diassembly_bytes);
                                assert(SUCCEEDED(hr));
                                assert(curr_results->disassembly != nullptr);
                                if (!SUCCEEDED(hr) || curr_results->disassembly == nullptr)
                                {
                                    std::stringstream msg;
                                    msg << kStrErrorDisassemblyContentsExtractionFailure <<
                                        kStrErrorPipelineIdentifiedByIndex << i << ".";
                                    error_msg.append(msg.str());
                                    ret = false;
                                }
                            }
                        ...
                        // Retrieve the statistics for the current pipeline.
                        if (ret && is_statistics_required)
                        {
                            AmdExtD3DShaderStatsRayTracing rt_stats_driver{};
                            hr = amd_shader_analyzer_ext_->GetRayTracingPipelineStatsByIndex(*pipeline_handle, i, &rt_stats_driver);
                            assert(SUCCEEDED(hr));
                            if (SUCCEEDED(hr))
                            {
                                ExtractRayTracingStats(*curr_results, rt_stats_driver);
                        ...
                        if (ret)
                        {
                            // Retrieve the buffer size.
                            uint32_t buffer_size = 0;
                            hr = amd_shader_analyzer_ext_->GetRayTracingPipelineElfBinary(*pipeline_handle, i, nullptr, &buffer_size);
                            assert(buffer_size > 0);
                            assert(SUCCEEDED(hr));
                            if (buffer_size > 0 && SUCCEEDED(hr))
                            {
                                ...
                                else
                                {
                                    std::stringstream msg;
                                    msg << kStrErrorPipelineBinaryExtractionFailure <<
                                        kStrErrorPipelineIdentifiedByIndexPipelineBinary << i;
                                }
                            }
                            else
                            {
                ...
                {
                    std::stringstream msg;
                    msg << kStrErrorDxrFailedToRetrievePipelineCount;
                    error_msg.append(msg.str());
                    ret = false;
                }
            }
        ...
        std::string& error_msg) const
    {
        ...
        if (impl_ != nullptr)
        {
            ret = impl_->CreateStateObject(ray_tracing_state_object, raygen_shader_name, raytracing_shader_stats, pipeline_binary, is_unified_mode, indirect_shader_names, error_msg);
        }
        return ret;
    }
    ...
    bool RgDx12Backend::CreateStateObjectAll(const RgDx12Config& config, const D3D12_STATE_OBJECT_DESC* ray_tracing_state_object,
        std::vector<std::shared_ptr<RgDx12ShaderResultsRayTracing>>& raytracing_shader_stats,
        std::vector<std::shared_ptr<std::vector<unsigned char>>>& pipeline_binaries, std::string& error_msg) const
    {
        bool ret = false;
        ...
        if (impl_ != nullptr)
        {
            ret = impl_->CreateStateObjectAll(config, ray_tracing_state_object, raytracing_shader_stats, pipeline_binaries, error_msg);
        }
        return ret;
    }
#endif
