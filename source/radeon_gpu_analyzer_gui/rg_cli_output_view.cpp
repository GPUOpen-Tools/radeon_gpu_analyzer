...
//
static bool  ParseErrorLocation(const QStringRef& loc, std::pair<QStringRef&, int&> parsed_loc)
{
    ...
    if (parent_offset != -1 && (column_offset = loc.lastIndexOf(kCharCmlprErrorDelimiter, parent_offset)) != -1)
    {
        ...
        {
            QStringRef  line_num_str = loc.mid(column_offset + 1, parent_offset - column_offset - 1);
    ...
    if (!found)
    {
            ...
            (offset = loc.indexOf(kStrCmplrNoteToken)) != -1)
        {
            ...
            {
                QStringRef  line_str = loc.mid(line_start_offset + 1, line_end_offset - line_start_offset - 1);
