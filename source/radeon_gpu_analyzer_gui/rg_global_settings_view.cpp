...
void RgGlobalSettingsView::PushToWidgets(const RgGlobalSettings& global_settings)
{
    ...
    QString lof_file_location(global_settings.log_file_location.c_str());
    ui_.logFileLocationLineEdit->setText(lof_file_location);
    ...
    ui_.defaultApiOnStartupComboBox->setCurrentIndex(global_settings.should_prompt_for_api ? static_cast<int>(RgProjectAPI::kUnknown) : static_cast<int>(global_settings.default_api));
...
void RgGlobalSettingsView::ConnectSignals()
{
    ...
    bool is_connected = connect(this->ui_.logFileLocationFolderOpenButton, &QPushButton::clicked, this, &RgGlobalSettingsView::HandleLogFileLocationBrowseButtonClick);
    assert(is_connected);
    ...
    is_connected = connect(ui_.defaultApiOnStartupComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &RgGlobalSettingsView::HandleComboBoxChanged);
    assert(is_connected);
    ...
    is_connected = connect(ui_.fontSizeComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &RgGlobalSettingsView::HandleComboBoxChanged);
    assert(is_connected);
    ...
    is_connected = connect(this->ui_.defaultLangComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &RgGlobalSettingsView::HandleComboBoxChanged);
    assert(is_connected);

    // Include files viewer browse button.
    is_connected = connect(this->ui_.includeFilesViewerBrowseButton, &QPushButton::clicked, this, &RgGlobalSettingsView::HandleIncludeFilesViewerBrowseButtonClick);
    assert(is_connected);

    // Include files viewer line edit.
    is_connected = connect(this->ui_.includeFilesViewerLineEdit, &QLineEdit::editingFinished, this, &RgGlobalSettingsView::HandleIncludeFilesViewerEditingFinished);
    assert(is_connected);
...
void RgGlobalSettingsView::HandleIncludeFilesViewerBrowseButtonClick(bool checked)
{
    ...
    bool is_selected = RgUtils::OpenFileDialog(this, selected_viewer_path,
        kStrFileDialogFilterIncludeViewer, kFileFilter) &&
        RgUtils::IsFileExists(selected_viewer_path);
...
void RgGlobalSettingsView::PopulateColumnVisibilityList()
{
    ...
    // Add the "All" entry.
    ListWidget::AddListWidgetCheckboxItem(kStrDisassemblyTableColumnAll, disassembly_columns_list_widget_, slot_function_pointer, this, kStrGlobalSettingsColumnVisibilityList, kStrGlobalSettingsColumnListItemAllCheckbox);
    ...
    for (int column_index = start_column; column_index < end_column; ++column_index)
    {
        ...
        ListWidget::AddListWidgetCheckboxItem(column_name.c_str(), disassembly_columns_list_widget_, slot_function_pointer, this, kStrGlobalSettingsColumnVisibilityList, kStrGlobalSettingsColumnListItemCheckbox);
    }
}
...
{
    return ui_.assocExtGlslLineEdit->text().isEmpty() ||
        ui_.assocExtHlslLineEdit->text().isEmpty() ||
        ui_.assocExtSpvasLineEdit->text().isEmpty() ||
