...
bool ParseCmdLine(int argc, char* argv[], Config& config)
{
    ...
    try
    {
        ...
        vulkan_opt.add_options()
            ...
            ("icd", po::value<string>(&config.icd_file), kStrCliOptIcdDescription)
            ("loader-debug", po::value<string>(&config.loader_debug), kStrCliOptVkLoaderDebugDescription)
        ...
        // Paths to user-provided LightningCompiler.
        po::options_description opencl_offline_compiler_paths_opt("Alternative OpenCL Lightning Compiler");
        opencl_offline_compiler_paths_opt.add_options()
            ("compiler-bin", po::value<string>(&config.compiler_bin_path), kStrCliDescAlternativeLightningCompilerBinFolder)
            ("compiler-inc", po::value<string>(&config.compiler_inc_path), kStrCliDescAlternativeLightningCompilerIncFolder)
            ("compiler-lib", po::value<string>(&config.compiler_lib_path), kStrCliDescAlternativeLightningCompilerLibFolder);
        ...
        vk_compiler_paths_opt.add_options()
            ("compiler-bin", po::value<string>(&config.compiler_bin_path), kStrCliDescAlternativeVkBinFolder);
        ...
        all_opt.add(generic_opt).add(macro_and_include_opt).add(cl_opt).add(hidden_opt).add(dx_opt).add(dx12_opt).add(dxr_opt).add(pipelined_opt_offline)
              .add(il_dump_opt).add(line_numbers_opt).add(warnings_opt).add(opt_level_opt2).add(opencl_offline_compiler_paths_opt).add(dxc_options).add(dx12_other_options);
        ...
        {
            config.mode = kModeOpenclOffline;
        }
        ...
        {
            config.mode = RgaMode::kModeOpenclOffline;
        }
        else
        {
        ...
        if ((config.requested_command == Config::kHelp) && (!is_source_specified))
        {
            ...
            cout << kStrRgaProductName << " " << kStrRgaVersionPrefix << kStrRgaVersion << "." << kStrRgaBuildNum << std::endl;
            ...
            cout << "To view help for OpenCL Offline mode: -h -s opencl" << std::endl;
#ifdef _LEGACY_OPENCL_ENABLED
        ...
        else if ((config.requested_command == Config::kHelp) && (config.mode == RgaMode::kModeOpenclOffline))
        {
            // Put all options valid for this mode to one group to make the description aligned.
            po::options_description  oclOptions;
            oclOptions.add(generic_opt).add(il_dump_opt).add(warnings_opt).add(macro_and_include_opt).add(cl_opt).add(line_numbers_opt).add(opt_level_opt2).add(opencl_offline_compiler_paths_opt);
            cout << "*** OpenCL Offline mode options ***" << endl;
            cout << "================================" << endl << endl;
            ...
            cout << "    " << program_name << " -s opencl --isa test_isa.txt --livereg regs.txt test.cl" << endl;
            cout << "  " << "List the kernels available in test.cl:" << endl;
            cout << "    " << program_name << " -s opencl --list-kernels test.cl" << endl;
            cout << "  " << "List the ASICs supported by the OpenCL Lightning Compiler that is bundled with the tool:" << endl;
            cout << "    " << program_name << " -s opencl --list-asics" << endl;
            cout << "  " << "Compile test.cl and extract ISA disassembly for gfx900 using alternative OpenCL compiler:" << endl;
            cout << "    " << program_name << " -s opencl -c gfx900 --isa test.isa --compiler-bin C:\\llvm\\dist\\bin --compiler-inc C:\\llvm\\dist\\include --compiler-lib C:\\llvm\\dist\\lib\\bitcode  test.cl" << endl;
