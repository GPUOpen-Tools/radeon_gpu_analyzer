    ...
    GT_IF_WITH_ASSERT(program_builder_dx != nullptr)
    {
        ...
        if (backend_rc == kBeStatusSuccess)
        {
            ...
            if (!config.inst_cfg_file.empty() || !config.block_cfg_file.empty())
            {
                ...
                KcUtils::GenerateControlFlowGraph(isa_output_filename.asASCIICharArray(), device_name, cfg_output_ilename.asASCIICharArray(), log_callback_,
...
{
    std::vector<char> binary = backend_handler_->theOpenDXBuilder()->GetDeviceBinaryElf(device_name);
    if (!binary.empty())
    {
...
void KcCliCommanderDX::RunCompileCommands(const Config& config, LoggingCallBackFuncP callback)
{
    ...
    if (is_init_successful)
    {
        ...
        {
            // Get the device name.
            const std::string device_name = devce_info.m_szCALName;
            if (kUnsupportedDevicesDx11.find(device_name) == kUnsupportedDevicesDx11.end())
            {
                if (Compile(config, devce_info, device_name))
                {
                    ...
                    {
                        is_isa_size_detected = ExtractStats(
                            device_name, config, should_detect_isa_size, isa_buffer, is_isa_size_detected, isa_size_bytes, analysis_data, device_analysis_data);
                    }
                }
            }
