...
/// @file
/// @brief Implementation for CLI Commander interface for compiling with the (legacy) Lightning Compiler (LC).
//=============================================================================
    ...
    if (ret)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                options.mode = kModeOpenclOffline;
...
{
    return KcCLICommanderLightning::ListEntriesOpenclOffline(config, callback);
}
        ...
        !config.block_cfg_file.empty() || !config.inst_cfg_file.empty() || !config.livereg_analysis_file.empty())
    {
        ...
        if (cl_builder != nullptr)
        {
            ...
            if (!config.isa_file.empty() || !config.il_file.empty())
            {
                ...
                for (const std::string& device_name : asics_)
                {
                    gtString device_name_gt_str;
                    device_name_gt_str << device_name.c_str();
                    ...
                    if (iter != device_to_code_object_disassembly_mapping.end())
                    {
                        ...
                        if (is_livereg_required)
                        {
                            if (is_post_processing_enabled)
                            {
                                ...
                                KcUtils::PerformLiveRegisterAnalysis(isa_output_filename, device_name_gt_str, liveRegAnalysisOutputFileName,
                        ...
                        if (is_cfg_required)
                        {
                            ...
                            if (is_post_processing_enabled)
                            {
                                if (!config.block_cfg_file.empty() || !config.inst_cfg_file.empty())
                                {
                                    ...
                                    // the analysis file.
                                    KcUtils::GenerateControlFlowGraph(isa_output_filename, device_name_gt_str, cfg_output_filename, log_callback_,
                            ...
                            if (status == kBeStatusSuccess)
                            {
                                ...
                                if (is_post_processing_enabled)
                                {
                                    if (is_livereg_required)
                                    {
                                        ...
                                        KcUtils::ConstructOutputFileName(config.livereg_analysis_file, kStrDefaultExtensionLivereg,
                                                                         ...
                                                                         kernel_name,
                                                                         device_name,
                                                                         livereg_analysis_output_filename);

                                        // Call the kcUtils routine to analyze <generatedFileName> and write the analysis file.
                                        bool is_livereg_performed = KcUtils::PerformLiveRegisterAnalysis(isa_output_filename,
                                                                                                         device_name_gt_str,
                                                                                                         livereg_analysis_output_filename,
                                                                                                         log_callback_,
                                ...
                                if (is_post_processing_enabled)
                                {
                                    if (!config.block_cfg_file.empty() || !config.inst_cfg_file.empty())
                                    {
                                        ...
                                        // Call the kcUtils routine to analyze <generatedFileName> and write
                                        // the analysis file.
                                        bool is_cfg_generated =  KcUtils::GenerateControlFlowGraph(isa_output_filename, device_name_gt_str, cfg_output_filename, log_callback_,
                                            !config.inst_cfg_file.empty(), config.print_process_cmd_line);
